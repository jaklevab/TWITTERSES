...
import sys
import pandas as pd
linkedin_data=pd.read_csv(header=0,index_col=0,filepath_or_buffer="/datastore/complexnet/jlevyabi/ml_soc_econ/data_files/UKSOC_rep/linkedin/linkedin_data/full_linked_info.csv")

geo_usrs=[str(x) for x in list(linkedin_data.twitter_id)]
...
def get_all_tweets(user_id):
	...
	#write the csv	
	with open('/datastore/complexnet/jlevyabi/ml_soc_econ/data_files/UKSOC_rep/tweets/linkedin/%s_tweets.csv'%user_id, 'w') as f:
		writer = csv.writer(f)
